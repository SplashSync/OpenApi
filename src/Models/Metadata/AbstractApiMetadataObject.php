<?php

/*
 *  This file is part of SplashSync Project.
 *
 *  Copyright (C) Splash Sync  <www.splashsync.com>
 *
 *  This program is distributed in the hope that it will be useful,
 *  but WITHOUT ANY WARRANTY; without even the implied warranty of
 *  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
 *
 *  For the full copyright and license information, please view the LICENSE
 *  file that was distributed with this source code.
 */

namespace Splash\OpenApi\Models\Metadata;

use Exception;
use Splash\Bundle\Models\AbstractStandaloneObject;
use Splash\Metadata\Models\Objects\MetadataFieldsTrait;
use Splash\Metadata\Models\Objects\MetadataObjectTrait;
use Splash\Metadata\Services\MetadataAdapter;
use Splash\Models\Objects\IntelParserTrait;
use Splash\Models\Objects\ListsTrait;
use Splash\Models\Objects\ObjectsTrait;
use Splash\Models\Objects\SimpleFieldsTrait;
use Splash\OpenApi\Hydrator\Hydrator;
use Splash\OpenApi\Models\Connexion\ConnexionInterface as Connexion;
use Splash\OpenApi\Visitor\AbstractVisitor as Visitor;
use Splash\OpenApi\Visitor\JsonVisitor;

/**
 * Abstract Class for Working with Splash Api Object & Metadata
 */
abstract class AbstractApiMetadataObject extends AbstractStandaloneObject
{
    //====================================================================//
    // Splash Core Traits
    use IntelParserTrait;
    use SimpleFieldsTrait;
    use ObjectsTrait;
    use ListsTrait;
    //====================================================================//
    // Splash Metadata Parser Traits
    use MetadataObjectTrait;
    use MetadataFieldsTrait;
    //====================================================================//
    // Splash OpenApi Metadata Traits
    use CRUDTrait;

    //====================================================================//
    // Open Api Variables
    //====================================================================//

    /**
     * Open Api Model Visitor
     */
    protected Visitor $visitor;

    /**
     * Class Constructor
     *
     * @param Connexion    $connexion
     * @param Hydrator     $hydrator
     * @param string       $objectClass
     * @param null|Visitor $visitor
     *
     * @throws Exception
     */
    public function __construct(
        protected readonly MetadataAdapter   $metadataAdapter,
        Connexion $connexion,
        Hydrator $hydrator,
        protected readonly string $objectClass,
        Visitor $visitor = null
    ) {
        $this->visitor = $visitor ?: new JsonVisitor($connexion, $hydrator, $objectClass);
    }

    //====================================================================//
    // Generic Splash Objects Methods
    //====================================================================//

    /**
     * {@inheritdoc}
     */
    public function objectsList(?string $filter = null, array $params = array()): array
    {
        return $this->visitor->list($filter, $params)->getArrayResults() ?? array();
    }
}
